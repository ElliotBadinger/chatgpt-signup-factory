...
describe('TC-4: inbox already registered → mark chatgpt-used, try next', () => {
  test('marks first inbox chatgpt-used and tries second inbox', async () => {
    ...
    // Matches the 5-evaluate-call sequence per chatGptAccountCreator.js.
    // Two browser sessions are created (one per inbox attempt):
    //  - Inbox 1 ("taken"): evaluate-1 returns alreadyRegistered=true → chatgpt-used
    //  - Inbox 2 ("fresh"): full 5-call happy path → success
    // Since SAME page object is reused, calls accumulate sequentially across sessions.
    let pageCall = 0;
    const page = {
      goto: jest.fn().mockResolvedValue({}),
      evaluate: jest.fn().mockImplementation(async () => {
        pageCall++;
        // ── Inbox 1: findSignupUrl + already-registered (2 evaluate calls) ──
        if (pageCall === 1) return null; // eval-0: findSignupUrlScript
        if (pageCall === 2) return { alreadyRegistered: true, emailFilled: false, url: 'https://chatgpt.com/' };
        // ── Inbox 2: full happy path (6 evaluate calls: 1 findSignup + 5 core) ──
        if (pageCall === 3) return null; // eval-0: findSignupUrlScript for second attempt
        if (pageCall === 4) return { alreadyRegistered: false, emailFilled: true, url: 'https://chatgpt.com/' };
        if (pageCall === 5) return { state: 'otp-needed', url: 'https://chatgpt.com/auth/verify' };
        if (pageCall === 6) return { otpFilled: true, nameFilled: false };
        if (pageCall === 7) return { clicked: true, btnText: 'Accept' };
        if (pageCall === 8) return {
