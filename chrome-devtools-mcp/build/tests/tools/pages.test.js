...
import { listPages, newPage, closePage, selectPage, navigatePage, resizePage, handleDialog, getTabId, } from '../../src/tools/pages.js';
...
describe('pages', () => {
    ...
    describe('new_page', () => {
        ...
        it('create a page in the background', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.strictEqual(await originalPage.evaluate(() => document.hasFocus()), true);
                await newPage.handler({ params: { url: 'about:blank', background: true } }, response, context);
    ...
    describe('close_page', () => {
        ...
        it('cannot close the last page', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.deepStrictEqual(response.responseLines[0], `The last open page cannot be closed. It is fine to keep it open.`);
                assert.ok(response.includePages);
                assert.ok(!page.isClosed());
            });
        });
    ...
    describe('select_page', () => {
        ...
        it('selects a page and keeps it focused in the background', async () => {
            await withMcpContext(async (response, context) => {
                await context.newPage();
                assert.strictEqual(context.getPageById(2), context.getSelectedPage());
                assert.strictEqual(await context.getPageById(1).evaluate(() => document.hasFocus()), false);
                await selectPage.handler({ params: { pageId: 1 } }, response, context);
                assert.strictEqual(context.getPageById(1), context.getSelectedPage());
                assert.strictEqual(await context.getPageById(1).evaluate(() => document.hasFocus()), true);
                assert.ok(response.includePages);
            });
        });
    });
    describe('navigate_page', () => {
        it('navigates to correct page', async () => {
            await withMcpContext(async (response, context) => {
                await navigatePage.handler({ params: { url: 'data:text/html,<div>Hello MCP</div>' } }, response, context);
                const page = context.getSelectedPage();
        ...
        it('throws an error if the page was closed not by the MCP server', async () => {
            await withMcpContext(async (response, context) => {
                ...
                try {
                    ...
                    assert.strictEqual(err.message, 'The selected page has been closed. Call list_pages to see open pages.');
                }
            });
        });
        it('go back', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.equal(await page.evaluate(() => document.location.href), 'about:blank');
                assert.ok(response.includePages);
            });
        });
        it('go forward', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await page.goto('data:text/html,<div>Hello MCP</div>');
                await page.goBack();
                await navigatePage.handler({ params: { type: 'forward' } }, response, context);
                assert.equal(await page.evaluate(() => document.querySelector('div')?.textContent), 'Hello MCP');
                assert.ok(response.includePages);
            });
        });
        it('reload', async () => {
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedPage();
                await page.goto('data:text/html,<div>Hello MCP</div>');
                await navigatePage.handler({ params: { type: 'reload' } }, response, context);
                assert.equal(await page.evaluate(() => document.location.href), 'data:text/html,<div>Hello MCP</div>');
                assert.ok(response.includePages);
            });
        });
        it('reload with accpeting the beforeunload dialog', async () => {
            ...
            window.addEventListener('beforeunload', e => {
              ...
              e.returnValue = '';
                ...
                assert.ok(response.includePages);
                assert.strictEqual(response.responseLines.join('\n'), 'Accepted a beforeunload dialog.\nSuccessfully reloaded the page.');
            });
        });
    ...
    describe('resize', () => {
        it('resize the page', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await resizePromise;
                await page.waitForFunction(() => window.innerWidth === 700 && window.innerHeight === 500);
        ...
        it('resize when window state is normal', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await resizePage.handler({ params: { width: 650, height: 450 } }, response, context);
                await resizePromise;
                await page.waitForFunction(() => window.innerWidth === 650 && window.innerHeight === 450);
                const dimensions = await page.evaluate(() => {
                    return [window.innerWidth, window.innerHeight];
                });
        ...
        it('resize when window state is minimized', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await resizePage.handler({ params: { width: 750, height: 550 } }, response, context);
                await resizePromise;
                await page.waitForFunction(() => window.innerWidth === 750 && window.innerHeight === 550);
                const dimensions = await page.evaluate(() => {
                    return [window.innerWidth, window.innerHeight];
                });
        ...
        it('resize when window state is maximized', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await resizePage.handler({ params: { width: 725, height: 525 } }, response, context);
                await resizePromise;
                await page.waitForFunction(() => window.innerWidth === 725 && window.innerHeight === 525);
                const dimensions = await page.evaluate(() => {
                    return [window.innerWidth, window.innerHeight];
                });
        ...
        it('resize when window state is fullscreen', async () => {
            await withMcpContext(async (response, context) => {
                ...
                await resizePage.handler({ params: { width: 850, height: 650 } }, response, context);
                await resizePromise;
                await page.waitForFunction(() => window.innerWidth === 850 && window.innerHeight === 650);
                const dimensions = await page.evaluate(() => {
                    return [window.innerWidth, window.innerHeight];
                });
