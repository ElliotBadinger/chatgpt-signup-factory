...
import { click, hover, fill, drag, fillForm, uploadFile, pressKey, clickAt, } from '../../src/tools/input.js';
...
describe('input', () => {
    ...
    describe('hover', () => {
        it('hovers', async () => {
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedPage();
                await page.setContent(html `<button onmouseover="this.innerText = 'hovered';">test</button>`);
                await context.createTextSnapshot();
                await hover.handler({
                    params: {
                        uid: '1_1',
                ...
                assert.strictEqual(response.responseLines[0], 'Successfully hovered over the element');
                assert.ok(response.includeSnapshot);
    ...
    describe('click_at', () => {
        it('clicks at coordinates', async () => {
          ...
          ></div>`);
                ...
                assert.strictEqual(response.responseLines[0], 'Successfully clicked at the coordinates');
                assert.ok(response.includeSnapshot);
                assert.ok(await page.$('text/clicked'));
            });
        });
        it('double clicks at coordinates', async () => {
          ...
          ></div>`);
                ...
                assert.strictEqual(response.responseLines[0], 'Successfully double clicked at the coordinates');
                assert.ok(response.includeSnapshot);
                assert.ok(await page.$('text/dblclicked'));
            });
        });
        ...
        it('fills out a textarea with long text', async () => {
            await withMcpContext(async (response, context) => {
                ...
                assert.ok(await page.evaluate(() => {
                    return (document.body.querySelector('textarea')?.value.length === 3_000);
                }));
            });
        });
    });
        ...
        it('drags one element onto another', async () => {
            ...
            <script>
              ...
              drop.addEventListener('drop', event => {
                ...
                assert.ok(response.includeSnapshot);
                assert.strictEqual(response.responseLines[0], 'Successfully dragged an element');
    ...
    describe('fill form', () => {
        it('successfully fills out the form', async () => {
            ...
            />
                ...
                assert.ok(response.includeSnapshot);
                assert.strictEqual(response.responseLines[0], 'Successfully filled out the form');
    ...
    describe('uploadFile', () => {
        it('uploads a file to a file input', async () => {
            ...
            />
                ...
                assert.ok(response.includeSnapshot);
                assert.strictEqual(response.responseLines[0], `File uploaded from ${testFilePath}.`);
            });
        ...
        it('uploads a file when clicking an element opens a file uploader', async () => {
            ...
            await withMcpContext(async (response, context) => {
                const page = context.getSelectedPage();
                await page.setContent(html `<button id="file-chooser-button">Upload file</button>
            <input
              type="file"
              id="file-input"
              style="display: none;"
        ...
        it('throws an error if the element is not a file input and does not open a file chooser', async () => {
            ...
            await withMcpContext(async (response, context) => {
                ...
                await context.createTextSnapshot();
                await assert.rejects(uploadFile.handler({
                    params: {
                        uid: '1_1',
                        filePath: testFilePath,
                    },
                }, response, context), {
                    message: 'Failed to upload file. The element could not accept the file directly, and clicking it did not trigger a file chooser.',
                });
