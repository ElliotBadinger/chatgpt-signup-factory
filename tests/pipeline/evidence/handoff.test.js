...
describe('writeHandoffBundle', () => {
  ...
  test('can send a Resend notification and records redacted delivery metadata', async () => {
    const artifactDir = await mkdtemp(path.join(os.tmpdir(), 'handoff-test-'));
    const fetchImpl = async () => ({
      ok: true,
      status: 200,
      text: async () => JSON.stringify({ id: 'email-789' }),
    });

    const result = await writeHandoffBundle(artifactDir, {
      ...baseHandoff,
      resend: {
        enabled: true,
    ...
    expect(result.notifications.resend).toMatchObject({
      status: 'sent',
      provider: 'resend',
      id: 'email-789',
      apiKey: 're_123...cdef',
    });
    expect(parsed.notifications.resend).toMatchObject(result.notifications.resend);
  });
});
