...
describe('TC-3: exhausted alias + available inbox → full rotation', () => {
  ...
  test('refreshes the emitted browserless audit artifact after replacement reconciliation', async () => {
    seed(healthPath, makeHealthFile({ alias1: 0 }));
    seed(routerPath, makeRouterFile(['alias1']));
    seed(authPath, makeAuth(['alias1']));
    seed(archivePath, emptyArchive());
    ...
    const result = await runCheckArchiveAndReplace({
      ...
      memberOnboarder: jest.fn().mockResolvedValue(
        makeMemberOnboarderResult('replacement@agentmail.to', 'workspace-123'),
      ),
      finalize: makeFinalizePersistingRouter(routerPath),
    ...
    expect(result.liveAudit.aliases.map((alias) => alias.aliasId)).toEqual(['replacement']);
    expect(result.liveAudit.aliases[0]).toEqual(expect.objectContaining({
      aliasId: 'replacement',
      email: 'replacement@agentmail.to',
      authPresent: true,
    }));

    const browserlessAudit = JSON.parse(fs.readFileSync(result.artifactPaths.browserlessAudit, 'utf8'));
    expect(browserlessAudit.liveAudit.aliases.map((alias) => alias.aliasId)).toEqual(['replacement']);
    expect(browserlessAudit.liveAudit.aliases[0]).toEqual(expect.objectContaining({
      aliasId: 'replacement',
      email: 'replacement@agentmail.to',
      authPresent: true,
    }));
...
// ─────────────────────────────── Yutori path ────────────────────────────────────
describe('yutoriCreateAccount: replaces createBrowserSession+createChatGptAccount', () => {
  test('when yutoriCreateAccount is provided it is called instead of createBrowserSession', async () => {
    seed(healthPath, makeHealthFile({ alias1: 0.0 }));
    seed(routerPath, makeRouterFile(['alias1']));
    seed(authPath, makeAuth(['alias1']));
    seed(archivePath, emptyArchive());
    seed(poolPath, {
      version: 1,
      entries: [makePoolEntry('inbox1@agentmail.to')],
      lastCheckedAt: 0, allEntriesExhausted: false,
    });

    const createBrowserSession = jest.fn();  // must NOT be called

    const yutoriCreateAccount = jest.fn().mockResolvedValue({
      success: true,
      auth: {
        type: 'oauth', access: 'yutori_access_tok',
        refresh: 'yutori_refresh_tok',
        expires: Date.now() + 3_600_000,
        accountId: 'yutori_uid_001',
      },
    });
    ...
    expect(result.newAccountsCreated).toBe(1);
    expect(yutoriCreateAccount).toHaveBeenCalledTimes(1);
    // The inbox object is passed as first arg; confirm it carries expected fields
    const [inboxArg] = yutoriCreateAccount.mock.calls[0];
    expect(inboxArg.inboxAddress).toBe('inbox1@agentmail.to');
    expect(inboxArg.rootApiKey).toBe('am_us_testkey123456');
    // createBrowserSession must NEVER be called when yutoriCreateAccount is set
    expect(createBrowserSession).not.toHaveBeenCalled();
  });

  test('when yutoriCreateAccount returns success:false the alias is counted as failed', async () => {
    seed(healthPath, makeHealthFile({ alias1: 0.0 }));
    seed(routerPath, makeRouterFile(['alias1']));
    seed(authPath, makeAuth(['alias1']));
    seed(archivePath, emptyArchive());
    ...
    const yutoriCreateAccount = jest.fn().mockResolvedValue({
      success: false,
      error: 'Yutori task failed: Cloudflare blocked',
    });
