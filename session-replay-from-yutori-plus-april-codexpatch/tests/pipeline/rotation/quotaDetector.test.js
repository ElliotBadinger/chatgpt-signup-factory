...
function buildHealthFile(modelEntries, providers = {}) {
...
describe('assessCodexQuotas', () => {
  ...
  // ── Blocker 1: provider cooldown+usage-limit overrides quotaProofAmbiguous ──────

  test('provider status cooldown+usage-limit marks alias exhausted even when quotaProofAmbiguous=true', () => {
    const aliasId = 'greenleaf';
    fs.writeFileSync(routerPath, JSON.stringify(buildRouterFile([
      { id: aliasId, cloneFrom: 'openai-codex', email: `${aliasId}@agentmail.to`, apiKey: 'unused', disabled: false },
    ])));
    fs.writeFileSync(healthPath, JSON.stringify(buildHealthFile(
      { [`${aliasId}/gpt-5.4`]: { quotaRemainingFraction: 1.0, quotaProofAmbiguous: true, quotaCheckedAt: Date.now() } },
      { [aliasId]: { status: 'cooldown', reason: 'usage-limit' } },
    )));
    const result = assessCodexQuotas({ healthPath, routerPath, maxStaleMs: 999_999_999 });
    expect(result.exhausted.length).toBe(1);
    expect(result.exhausted[0].aliasId).toBe(aliasId);
    expect(result.exhausted[0].exhausted).toBe(true);
    expect(result.exhausted[0].ambiguous).toBe(true); // still flagged ambiguous for info
  });

  test('provider status cooldown+usage-limit marks multiple aliases exhausted', () => {
    const aliases = ['greenleaf', 'cheerfulinformation738', 'motionlessfloor327'];
    fs.writeFileSync(routerPath, JSON.stringify(buildRouterFile(
      aliases.map((id) => ({ id, cloneFrom: 'openai-codex', email: `${id}@agentmail.to`, apiKey: 'unused', disabled: false })),
    )));
    const providers = {
      greenleaf: { status: 'cooldown', reason: 'usage-limit' },
      cheerfulinformation738: { status: 'cooldown', reason: 'usage-limit' },
      motionlessfloor327: { status: 'ok', reason: null },  // healthy
    };
    const models = {};
    aliases.forEach((id) => {
      models[`${id}/gpt-5.4`] = { quotaRemainingFraction: 1.0, quotaProofAmbiguous: true, quotaCheckedAt: Date.now() };
    });
    fs.writeFileSync(healthPath, JSON.stringify(buildHealthFile(models, providers)));
    const result = assessCodexQuotas({ healthPath, routerPath, maxStaleMs: 999_999_999 });
    expect(result.exhausted.length).toBe(2);
    expect(result.exhausted.map((a) => a.aliasId).sort()).toEqual(['cheerfulinformation738', 'greenleaf'].sort());
    expect(result.healthy.length).toBe(1);
    expect(result.healthy[0].aliasId).toBe('motionlessfloor327');
  });

  test('provider status cooldown with non-usage-limit reason does NOT mark exhausted', () => {
    // http-429 is rate-limiting, not necessarily exhausted
    const aliasId = 'ratelimited';
    fs.writeFileSync(routerPath, JSON.stringify(buildRouterFile([
      { id: aliasId, cloneFrom: 'openai-codex', email: `${aliasId}@agentmail.to`, apiKey: 'unused', disabled: false },
    ])));
    fs.writeFileSync(healthPath, JSON.stringify(buildHealthFile(
      { [`${aliasId}/gpt-5.4`]: { quotaRemainingFraction: 0.50, quotaProofAmbiguous: false, quotaCheckedAt: Date.now() } },
      { [aliasId]: { status: 'cooldown', reason: 'http-429' } },
    )));
    const result = assessCodexQuotas({ healthPath, routerPath, maxStaleMs: 999_999_999 });
    expect(result.exhausted.length).toBe(0);
    expect(result.healthy.length).toBe(1);
  });

  test('alias with no provider entry but low fraction still exhausted normally', () => {
    const aliasId = 'noprovider';
    fs.writeFileSync(routerPath, JSON.stringify(buildRouterFile([
      { id: aliasId, cloneFrom: 'openai-codex', email: `${aliasId}@agentmail.to`, apiKey: 'unused', disabled: false },
    ])));
    fs.writeFileSync(healthPath, JSON.stringify(buildHealthFile(
      { [`${aliasId}/gpt-5.4`]: { quotaRemainingFraction: 0.0, quotaProofAmbiguous: false, quotaCheckedAt: Date.now() } },
