...
describe('runBootstrap', () => {
  ...
  test('live mode reruns a controller left in dry-run ready state instead of skipping it', async () => {
    const stateDir = await mkdtemp(path.join(os.tmpdir(), 'pipeline-bootstrap-'));
    const artifactDir = path.join(stateDir, 'artifacts', 'bootstrap');
    const writeHandoffBundle = jest.fn(async () => {});
    const existingController = {
      id: 'controller-stale-example-com',
      email: 'stale@example.com',
      status: 'ready',
      mode: 'dry-run',
      successfulInviteCount: 0,
      createdAt: '2026-03-13T14:20:00.000Z',
      updatedAt: '2026-03-13T14:20:00.000Z',
    };
    await writeFile(
      path.join(stateDir, 'controller_registry.json'),
      `${JSON.stringify([existingController], null, 2)}\n`,
      'utf8',
    );
    ...
    const result = await runBootstrap({
      ...
      artifactDir,
      writeHandoffBundle,
      candidateRootEmails: ['stale@example.com'],
      now: () => '2026-03-13T14:25:00.000Z',
      verifyMailboxAuthority: jest.fn(async ({ controller }) => {
        calls.push(['verifyMailboxAuthority', controller.id]);
        return { authority: 'verified', artifactPath: '/evidence/mailbox.json' };
      }),
      createOrRecoverAgentMailController: jest.fn(async ({ controller }) => {
        calls.push(['createOrRecoverAgentMailController', controller.id]);
        return { agentMailId: 'am-2', artifactPath: '/evidence/controller.json' };
      }),
      captureApiKey: jest.fn(async ({ controller }) => {
        calls.push(['captureApiKey', controller.id]);
        return { keyCaptured: true, artifactPath: '/evidence/key.json' };
      ...
      createInboxes: jest.fn(async ({ controller }) => {
        calls.push(['createInboxes', controller.id]);
        return { inboxIds: ['inbox-3'], artifactPath: '/evidence/inboxes.json' };
      }),
    });

    expect(calls).toEqual([
      ['verifyMailboxAuthority', 'controller-stale-example-com'],
      ['createOrRecoverAgentMailController', 'controller-stale-example-com'],
      ['captureApiKey', 'controller-stale-example-com'],
      ['createInboxes', 'controller-stale-example-com'],
    ]);
    expect(result.controllers).toEqual([
      expect.objectContaining({
        id: 'controller-stale-example-com',
