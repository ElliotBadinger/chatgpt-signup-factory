...
export function assessCodexQuotas({
  ...
  const assessed = aliases.map((alias) => {
    ...
    // Provider-status override: if the account-router extension has flagged this
    // alias as cooldown+usage-limit in providers{}, treat it as exhausted regardless
    // of quotaProofAmbiguous. This covers the common case where all codex aliases
    // show fraction=1/ambiguous=true because the quota probe captured only one window,
    // but the provider status clearly says usage-limit is hit.
    const providers = healthData.providers ?? {};
    const providerEntry = providers[alias.id];
    const isCooldownExhausted =
      providerEntry?.status === 'cooldown' &&
      providerEntry?.reason === 'usage-limit';

    const exhausted = isCooldownExhausted ||
