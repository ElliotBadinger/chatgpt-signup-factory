...
import { createPipelineStore, loadJsonFile } from '../state/store.js';
...
function buildBootstrapResumeModeFlag(controller) {
  return controller.bootstrapMode === 'live'
    ? ' --live'
    : controller.bootstrapMode === 'dry-run'
      ? ' --dry-run'
      : '';
}

function buildBootstrapHandoff({ stateDir, artifactDir, controller, outputs = {}, proofPaths, blocker }) {
  const target = controller.email ?? controller.id;
  const modeFlag = buildBootstrapResumeModeFlag(controller);

  return {
    target,
    inviter: 'bootstrap',
    inviteLink: '',
    proofPaths: Array.isArray(proofPaths) ? proofPaths : extractProofPaths(outputs),
    status: controller.status,
    blocker,
    resumeCommand: `node src/cli/pipeline-bootstrap.js --state-dir ${stateDir} --artifact-dir ${artifactDir}${modeFlag} --root ${target}`,
    statusCommand: `node src/cli/pipeline-status.js --state-dir ${stateDir}`,
  };
}

async function loadExistingBootstrapProofPaths(artifactDir, controller) {
  const summary = await loadJsonFile(path.join(buildBootstrapArtifactDir(artifactDir, controller), 'summary.json'), null);
  return Array.isArray(summary?.proofPaths) ? summary.proofPaths : undefined;
}

async function writeBootstrapHandoff({ stateDir, artifactDir, controller, outputs, proofPaths, blocker, writeHandoffBundle }) {
  if (!artifactDir) {
    return;
  }
  ...
  await writeHandoffBundle(
    ...
    buildBootstrapHandoff({ stateDir, artifactDir, controller, outputs, proofPaths, blocker }),
  );
}
...
export async function runBootstrap({
  ...
  for (const candidateEmail of candidateRootEmails) {
    ...
    if (existingController && isBootstrapComplete(existingController.status) && existingController.bootstrapMode === 'live') {
      if (artifactDir) {
        await writeBootstrapHandoff({
          stateDir,
          artifactDir,
          controller: existingController,
          proofPaths: await loadExistingBootstrapProofPaths(artifactDir, existingController),
          writeHandoffBundle,
        });
      }
      continue;
