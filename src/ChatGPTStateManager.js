export class ChatGPTStateManager {
  detectState(snapshot) {
    ...
    if (snapshot.includes("You're all set") || snapshot.includes('You’re all set') || snapshot.includes('button "Skip"') || snapshot.includes('button "Next"') || snapshot.includes('Okay, let’s go') || snapshot.includes('Stay logged in') || (snapshot.includes('button "Continue"') && !snapshot.includes('textbox'))) {
      return 'ONBOARDING';
    }
