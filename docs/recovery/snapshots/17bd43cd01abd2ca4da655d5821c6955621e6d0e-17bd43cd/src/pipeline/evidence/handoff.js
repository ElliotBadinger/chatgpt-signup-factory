...
export async function writeHandoffBundle(artifactDir, handoff) {
  ...
  await Promise.all([
    writeFile(path.join(artifactDir, 'handoff.md'), renderHandoffMarkdown(handoff), 'utf8'),
    writeFile(path.join(artifactDir, 'commands.sh'), renderCommandsShell(handoff), 'utf8'),
    writeSummaryJson(artifactDir, {
