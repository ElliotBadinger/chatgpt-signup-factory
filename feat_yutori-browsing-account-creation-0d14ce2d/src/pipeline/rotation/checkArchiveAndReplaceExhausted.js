 ...
 *   yutoriCreateAccount?:  (inbox: object, opts?: object) => Promise<{ success: boolean, auth?: object, error?: string }>;
...
export async function runCheckArchiveAndReplace({
  ...
  const rawResults = await runWithConcurrency(workItems, concurrency, async ({ alias }) => {
    ...
    for (let attempt = 0; attempt < MAX_INBOX_ATTEMPTS; attempt++) {
      ...
      if (yutoriCreateAccount) {
        // ── Yutori Browsing API path ─────────────────────────────────────────
        // Pre-invite is handled inside yutoriCreateAccount (called before task launch).
        // No local browser session needed.
        try {
          createResult = await yutoriCreateAccount(inbox, {
