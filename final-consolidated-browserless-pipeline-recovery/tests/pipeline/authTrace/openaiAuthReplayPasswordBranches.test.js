...
describe('replayOpenAiAuthFlow password branches', () => {
  ...
  test('existing-login OTP fallback uses the passwordless send-otp endpoint from password login', async () => {
    const requests = [];
    const fetchImpl = async (url, options = {}) => {
      ...
      if (url === 'https://auth.openai.com/api/accounts/passwordless/send-otp') {
        ...
        return {
          ...
          text: async () => JSON.stringify({ error: { message: 'Invalid session. Please start over.', code: 'invalid_state' } }),
        };
      }
      return makePasswordBootstrapFetch()(url, options);
    };
    ...
    expect(result.verdict).toBe('passwordless-send-failed');
    expect(requests).toContainEqual({
      url: 'https://auth.openai.com/api/accounts/passwordless/send-otp',
      method: 'POST',
    });
    expect(requests.some((request) => request.url === 'https://auth.openai.com/api/accounts/email-otp/send')).toBe(false);
  });
});
