...
export class RealChromeAgentMailDriver {
  constructor({
    env,
    mailboxAuthority,
    chromePath,
  ...
  async ensureOrganizationSelected(page) {
    ...
    // If Clerk setActive didn't work, or it left us stranded on /select-organization,
    // fall back to clicking the org button, which completes the dashboard transition.
    if (result.action !== 'set-active' || page.url().includes('/select-organization')) {
