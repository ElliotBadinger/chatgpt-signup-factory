import { createCookieJar, renderCookieHeader, snapshotCookies } from '../authTrace/httpCookies.js';
  ...
  async function request(url, {
    ...
    for (let attempt = 1; attempt <= Math.max(1, maxAttempts); attempt += 1) {
      try {
        ...
        if (attempt >= Math.max(1, maxAttempts) || !isTransientFetchError(error)) {
          throw error;
        }
