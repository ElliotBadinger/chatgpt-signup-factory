...
export function promoteAuthCredentialAlias({
  fromAliasId,
  toAliasId,
  authJsonPath = AUTH_JSON_PATH,
}) {
  const data = loadJson(authJsonPath, {});
  if (!data[fromAliasId]) return false;
  data[toAliasId] = data[fromAliasId];
  delete data[fromAliasId];
  atomicWrite(authJsonPath, data);
  return true;
}
...
export function removeAliasRouterState(aliasId, {
  poolName = '*',
  routerJsonPath = ROUTER_JSON_PATH,
} = {}) {
  return deregisterAlias(aliasId, { poolName, routerJsonPath });
}
