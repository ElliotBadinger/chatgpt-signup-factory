...
async function tryOwnedOauthPasswordLogin({
  email,
  password,
  fallbackEntryUrl,
  jar,
  ...
  if (!email || !password || !fallbackEntryUrl) return null;
  const passwordLogin = await performRequest({
    jar,
    ...
    fetchImpl,
    now,
    stepName: "owned_oauth_password_login",
    redactRequestBody: true,
  });
  steps.push(passwordLogin.step);

  if (passwordLogin.response.status >= 400) return null;
  const continueUrl = resolveNavigationUrl(
    passwordLogin.responseJson?.continue_url ?? passwordLogin.response.headers.get("location") ?? null,
    { baseUrl: fallbackEntryUrl, redirectUri },
  );
  if (!continueUrl) return null;
  if (isExpectedRedirect(continueUrl, redirectUri)) return continueUrl;
  return followOwnedOauthContinuationToCallback({
    jar,
    startUrl: continueUrl,
    fetchImpl,
    now,
    redirectUri,
    steps,
...
export async function acquireOwnedOpenAiOauth({
  ...
  if (!callbackUrl && fallbackEntryUrl) {
    callbackUrl = await tryOwnedOauthPasswordLogin({
      email,
      password: buildSignupPassword(email),
      fallbackEntryUrl,
      jar,
      fetchImpl,
      now,
    ...
    const passwordlessSendOtpResult = !callbackUrl ? await performRequest({
      jar,
    ...
    if (passwordlessSendOtpResult) steps.push(passwordlessSendOtpResult.step);

    if (
      passwordlessSendOtpResult &&
