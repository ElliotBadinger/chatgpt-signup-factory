...
async function runPasswordlessOtpFromPasswordPage({
  ...
  try {
    ...
    return {
      ...
      blockerDetail: String(error?.message ?? error),
    };
  }
...
export async function replayOpenAiAuthFlow({
  ...
  if (redirectLocation.includes('/log-in/password')) {
    if (mode === 'existing-login-otp') {
      const passwordlessResult = await runPasswordlessOtpFromPasswordPage({
        email,
        redirectLocation,
        jar,
        authorizeUrl,
      ...
      const verdict = passwordlessResult.verdict === 'authenticated'
        ? 'authenticated'
        : (passwordlessResult.blockerReason === 'passwordless-otp-unsupported'
            ? 'passwordless-send-failed'
            : (passwordlessResult.blockerReason ?? passwordlessResult.verdict));
      return {
        ...buildReplayResult({
          branch: 'existing-login-otp',
          verdict,
          steps,
          jar,
          sessionJson: passwordlessResult.sessionJson ?? null,
          now,
          startedAt,
        }),
        blockerReason: passwordlessResult.blockerReason ?? null,
      };
    }
  ...
  if (redirectLocation.includes('/create-account/password')) {
    ...
    if (register.status >= 400) {
      return {
        ...
        blockerReason: register.responseJson?.error?.code
          ?? register.responseJson?.error?.message
    ...
    if (emailOtpSend.status >= 400) {
      return {
        ...buildReplayResult({
          branch: 'signup-new',
          verdict: 'signup-otp-send-failed',
          steps,
          jar,
          sessionJson: null,
          now,
        ...
        blockerReason: emailOtpSend.responseJson?.error?.code
          ?? emailOtpSend.responseJson?.error?.message
          ?? `email-otp-send-status-${emailOtpSend.status}`,
        finalSession: createEmptySession(),
      };
    }
    ...
    try {
      ...
      return {
        ...
        blockerReason: 'signup-otp-missing',
        blockerDetail: String(error?.message ?? error),
        finalSession: createEmptySession(),
      };
    }
